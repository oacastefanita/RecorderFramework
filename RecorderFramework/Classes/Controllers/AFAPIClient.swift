...
public class AFAPIClient : NSObject {
    ...
    public static let sharedInstance = AFAPIClient()
...
//                                AFAPIClient.sharedInstance.downloadAudioFile(synchedItem, toFolder:recordFolder.title, completionHandler:{ (Bool success) -> Void in
//                                    AppPersistentData.sharedInstance.saveData()
//                                });
//                            }
//                        }
        ...
        _ = manager?.post("get_phones", parameters: parameters, success: { (operation: AFHTTPRequestOperation!, response:Any!) -> Void in
            ...
            if let jsonDict = try JSONSerialization.jsonObject(with: operation.responseData, options: JSONSerialization.ReadingOptions.mutableLeaves) as? NSDictionary {
                ...
                for phoneNumber in  AppPersistentData.sharedInstance.phoneNumbers {
                    ...
                    if !FileManager.default.fileExists(atPath: path) {
                        AFAPIClient.sharedInstance.downloadFile(phoneNumber.flag! as NSString, localPath:path as NSString, completionHandler: { (success) -> Void in
        ...
        _ = manager?.post("delete_folder", parameters: parameters, success: { (operation: AFHTTPRequestOperation!, response:Any!) -> Void in
            do {
                ...
                if (jsonDict.count == 0 || (jsonDict.count > 0 && jsonDict.object(forKey: "status") != nil && !(jsonDict.object(forKey: "status")! as AnyObject).isEqual("ok"))) {
                    ...
                    if completionHandler != nil {
                        completionHandler!( true, nil)
                        AFAPIClient.sharedInstance.updateFolders({ (success) -> Void in
...
//        op.outputStream.open()
    ...
    public func downloadAudioFile(_ recordItem:RecordItem, toFolder:String, completionHandler:((Bool) -> Void)?) {
        ...
        if !FileManager.default.fileExists(atPath: path) {
            AFAPIClient.sharedInstance.downloadFile(recordItem.url! as NSString, localPath:path as NSString, completionHandler: { (success) -> Void in
                    ...
                    self.getMetadataFiles(recordItem, completionHandler: { (success, files) -> Void in
                        if success && files != nil {
                            ...
                            for file in allFiles{
                                ...
                                AFAPIClient.sharedInstance.downloadFile(url as NSString, localPath:metaPath as NSString, completionHandler: { (success) -> Void in
                                    if(success){
...
//            AppPersistentData.sharedInstance.apiKey = "557872b508520557872b50855c"
        ...
        AFAPIClient.sharedInstance.getSettings({ (success, data) -> Void in
            AFAPIClient.sharedInstance.getMessages({ (success, data) -> Void in
                AFAPIClient.sharedInstance.getLanguages { (success, data) -> Void in
                    AFAPIClient.sharedInstance.getTranslations(TranslationManager.sharedInstance.currentLanguage, completionHandler:{ (success, data) -> Void in
                        AFAPIClient.sharedInstance.getPhoneNumbers { (success, data) -> Void in
                            if !success {
                                self.mainSyncErrors += 1
                            }
                            AFAPIClient.sharedInstance.getFolders({ (success, data) -> Void in
                                if !success {
                                    self.mainSyncErrors += 1
                                }
    ...
    public func updateFolders(_ completionHandler:((Bool) -> Void)?) {
        AFAPIClient.sharedInstance.getFolders { (success, data) -> Void in
            ...
            AFAPIClient.sharedInstance.getRecordings({ (success) -> Void in
                if completionHandler != nil {
                    completionHandler!(true)
                }
            })
    ...
    public func getRecordings(_ completionHandler:((Bool) -> Void)?) {
        ...
        for recordFolder in RecordingsManager.sharedInstance.recordFolders {
            ...
            AFAPIClient.sharedInstance.getRecordings(recordFolder.id, completionHandler:{ (success, data) -> Void in
                if !success {
                    self.mainSyncErrors += 1
                }
                countToHandle -= 1
