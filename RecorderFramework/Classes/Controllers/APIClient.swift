...
public let API_BASE_URL = "https://app.recordphonecalls.co/rapi/"
...
public class APIClient : NSObject {
    ...
    func createFolder(_ name:String, localID:String , color:String = "0", pass:String! = nil, completionHandler:((Bool, Any?) -> Void)?)
    {
        ...
        api.doRequest(ServerPaths.createFolder.rawValue, method: .post, parameters: APIRequestParametersController.createCreateFolderParameters(name:name, localID:localID, color:color, pass:pass)) { (success, data) in
            if success {
                ...
                else if let intColor = Int(color), let folderColor = FolderColor(rawValue: intColor) {
                    recordFolder?.color = folderColor
                }
    ...
    func uploadMetadataFile(_ recordItem:RecordItem, oldId:String! = nil, completionHandler:((Bool, Any?) -> Void)?) {
        ...
        var path = fileManager.containerURL(forSecurityApplicationGroupIdentifier: RecorderFrameworkManager.sharedInstance.containerName)!.path
        path += recordItem.localFile
        if (recordItem.localFile.count == 0){
            return
        }
