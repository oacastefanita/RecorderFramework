...
class ViewTagViewController: UIViewController,UITableViewDelegate, UITableViewDataSource{
...
//                            fatalError("Failed to get mesh from asset.")
        ...
        case .imageURL:
            ...
            if self.tag.arg != nil{
                if self.tag.arg != nil{
                    self.lblArg1.text = self.tag.arg as! String
                    var path = RecorderFrameworkManager.sharedInstance.getPath() + (self.tag.arg as! String).components(separatedBy: "/").last!
                    RecorderFrameworkManager.sharedInstance.downloadFile(self.tag.arg as! String, atPath: path, completionHandler: { (success, data) -> Void in
                        if success {
